...
push $target_image=image_name $tag=default_tag:
    #!/usr/bin/env bash
    set -euo pipefail
    sudo podman login ghcr.io -u {{ repo_organization }}
    sudo podman push localhost/{{ target_image }}:{{ tag }} ghcr.io/{{ repo_organization }}/{{ target_image }}:{{ tag }}

# Sign image with cosign (requires cosign.key)
sign $target_image=image_name $tag=default_tag:
    #!/usr/bin/env bash
    set -euo pipefail
    if [[ ! -f "cosign.key" ]]; then
        echo "Error: cosign.key not found in repo root"
        exit 1
    fi
...
# Build, rechunk, push, and sign image (full release workflow)
release $target_image=image_name $tag=default_tag:
    ...
    just sign {{ target_image }} {{ tag }}
    echo "✓ Image {{ target_image }}:{{ tag }} built, rechunked, pushed, and signed"
